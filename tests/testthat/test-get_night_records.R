test_that("get night records works", {
  library(zoo)
  library(purrr)
  library(dplyr)
  library(tidyr)
...
test_that("get night records stops without seconds", {
  library(zoo)
  library(purrr)
  library(dplyr)
  library(tidyr)
  ...
  # TODO change the code to be slicedTsibble as opposed to sampled
  d1t0_all_room_day <- d1t0_overall_interval |>
    mutate(night = map(slicedTsibble, ~ expect_error(get_night_records(.x,"04:00","22:00"))))
  # check n day records
